...
class KhoiThiService {
  getDanhSachKhoiThi() {
    return axios
      .get("/khoithis", {params: {size: '10000'}}, { headers: authHeader() })
      .then((response) => {
  ...
  getDanhSachKhoiThiCuaCuocThi(cuocThiId) {
    return axios
      .get("/cuocthis/" + cuocThiId + "/khoithis", {params: {size: '10000'}}, { headers: authHeader() })
      .then((response) => {
        return response.data;
      });
  }
  themKhoiThi(cuocThiId, khoiThi) {
    return axios
      .post("/cuocthis/" + cuocThiId + "/khoithis", khoiThi, {
        headers: authHeader(),
      })
      .then((response) => {
        return response.data;
